...
local scale = (ScrW() >= 2560 and (ScrW() / baseWidth)) or (1 / (baseWidth / ScrW())) or 0.8
...
local c00200200 = Color(0, 0, 0, 200)
...
hook.Add("HUDPaint", "wepsel", function()
	...
	local offx = ScrW() / 2 - thisWidth / 2

	for i, v in pairs(loadout) do
		local offy = ScrH()/5 - 175
		...
		for j, wep in pairs(v) do
			...
			surface.SetDrawColor(selected and c00200200 or (wep.new and Color(0, 0, math.abs(math.sin(RealTime())) * 255, 150) or c000127))
