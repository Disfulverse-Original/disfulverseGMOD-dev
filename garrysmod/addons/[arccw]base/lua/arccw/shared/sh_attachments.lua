...
function ArcCW:PlayerGiveAtt(ply, att, amt)
    ...
    sql.Query(string.format("INSERT INTO player_attachments (steamid, attachment_name, count) VALUES ('%s', '%s', %d)", ply:SteamID(), att, amt))
...
local tableName = "player_attachments"

-- Check if the table already exists
local function TableExists(tableName)
    local query = "SELECT name FROM sqlite_master WHERE type='table' AND name='" .. tableName .. "';"
...
local function CreateTable(tableName)
    local query = [[
        CREATE TABLE IF NOT EXISTS ]] .. tableName .. [[ (
            ...
            PRIMARY KEY (steamid, attachment_name)
        );
    ]]
    sql.Query(query)
end
...
--gpt
hook.Add("PlayerInitialSpawn", "LoadPlayerAttachments", function(ply)
    local steamid = ply:SteamID()

    -- Retrieve attachment data from the database
    local result = sql.Query(string.format("SELECT attachment_name, count FROM player_attachments WHERE steamid = '%s'", steamid))

    if result then
        for _, row in ipairs(result) do
            local attachmentName = row.attachment_name
            local count = tonumber(row.count) or 0

            -- Give attachments back to the player
            for i = 1, count do
                ArcCW:PlayerGiveAtt(ply, attachmentName)
            end
        end
        print("All Attachments loaded for: " .. ply:GetName())
    end
end)
--gpt
