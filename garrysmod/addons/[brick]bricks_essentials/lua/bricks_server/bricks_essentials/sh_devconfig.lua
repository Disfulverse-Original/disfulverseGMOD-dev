...
if( BRICKS_SERVER.Func.IsSubModuleEnabled( "essentials", "crafting" ) ) then
    ...
    BRICKS_SERVER.DEVCONFIG.CraftingTypes = {
        ...
        ["Craft Tool"] = { 
            ReqInfo = {
                [1] = { "Tool", "table", "crafttools", function( itemTable )
                    if( itemTable.ReqInfo and itemTable.ReqInfo[1] and BRICKS_SERVER.CONFIG.CRAFTING.CraftTools[itemTable.ReqInfo[1]] ) then
                        itemTable.Model = BRICKS_SERVER.CONFIG.CRAFTING.CraftTools[itemTable.ReqInfo[1]][1]
                        --if( BRICKS_SERVER.CONFIG.CRAFTING.Resources[itemTable.ReqInfo[1]][2] ) then
                            --itemTable.Color = BRICKS_SERVER.CONFIG.CRAFTING.Resources[itemTable.ReqInfo[1]][2]
                        --end
                    end
            ...
            OnCraft = function( ply, reqInfo, itemInfo )
                if( not BRICKS_SERVER.CONFIG.CRAFTING.CraftTools[reqInfo[1] or ""] ) then return false end
                
                --PrintTable(itemInfo)
                local itemData = { "bricks_server_crafttool", (BRICKS_SERVER.CONFIG.CRAFTING.CraftTools[reqInfo[1] or ""][1] or ""), reqInfo[1] }
                itemData[4] = "bricks_server_crafttool_" .. string.Replace( string.lower( reqInfo[1] or "" ), " ", "" )
                ply:BRS():AddInventoryItem( itemData, (reqInfo[2] or 1) )
            end
        },
        ["Blueprint"] = { 
            ReqInfo = {
                [1] = { "Blueprint", "table", "blueprints", function( itemTable )
                    if( itemTable.ReqInfo and itemTable.ReqInfo[1] and BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[itemTable.ReqInfo[1]] ) then
                        itemTable.Model = BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[itemTable.ReqInfo[1]][1]
                        --if( BRICKS_SERVER.CONFIG.CRAFTING.Resources[itemTable.ReqInfo[1]][2] ) then
                            --itemTable.Color = BRICKS_SERVER.CONFIG.CRAFTING.Resources[itemTable.ReqInfo[1]][2]
                        --end
                    end
            ...
            OnCraft = function( ply, reqInfo, itemInfo )
                if( not BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[reqInfo[1] or ""] ) then return false end

                --PrintTable(BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[reqInfo[1]])
                --print(reqInfo[1])
                --PrintTable(reqInfo)
                local itemData = { "bricks_server_blueprint", (BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[reqInfo[1] or ""][1] or ""), reqInfo[1] }

                itemData[3] = BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[reqInfo[1]][4]
                itemData[4] = reqInfo[1]
                itemData[5] = BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[reqInfo[1]][2]
                itemData[6] = BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[reqInfo[1]][3]
...
if( BRICKS_SERVER.Func.IsSubModuleEnabled( "essentials", "boss" ) ) then
    ...
    BRICKS_SERVER.DEVCONFIG.LootTypes = {
        ...
        ["Craft Tools"] = { 
            ReqInfo = {
                [1] = { "Craft Tool", "table", "crafttools", function( itemTable )
                    if( itemTable.ReqInfo and itemTable.ReqInfo[1] and BRICKS_SERVER.CONFIG.CRAFTING.CraftTools[itemTable.ReqInfo[1]] ) then
                        itemTable.Model = BRICKS_SERVER.CONFIG.CRAFTING.CraftTools[itemTable.ReqInfo[1]][1]
                        --if( BRICKS_SERVER.CONFIG.CRAFTING.Resources[itemTable.ReqInfo[1]][2] ) then
                            --itemTable.ModelColor = BRICKS_SERVER.CONFIG.CRAFTING.Resources[itemTable.ReqInfo[1]][2]
        ...
        ["Blueprints"] = { 
            ReqInfo = {
                [1] = { "Blueprint", "table", "blueprints", function( itemTable )
                    if( itemTable.ReqInfo and itemTable.ReqInfo[1] and BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[itemTable.ReqInfo[1]] ) then
                        itemTable.Model = BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[itemTable.ReqInfo[1]][1]
                        if( BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[itemTable.ReqInfo[1]][2] ) then
                            itemTable.ModelColor = BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[itemTable.ReqInfo[1]][2]
                        end
                    end
                end },
                [2] = { "Amount", "integer" }
            ...
            GiveFunction = function(ply, reqInfo, itemInfo)
                ...
                local blueprintData = BRICKS_SERVER.CONFIG.CRAFTING.Blueprints[reqInfo[1]]

                local modelPath = blueprintData[1] or ""  -- Path to the blueprint model
                local maxAmount = blueprintData[2] or 0   -- Amount required (e.g., quantity)
                local currAmount = blueprintData[3] or 0   -- Amount required (e.g., quantity)
                local blueprintName = blueprintData[4] or ""

                local itemData = {
                    "bricks_server_blueprint",  -- Item type (assuming blueprint)
                    modelPath,  -- Model path associated with the blueprint
                    blueprintName,  -- Name of the blueprint
                    maxAmount,  -- Amount required (e.g., quantity)
                    currAmount,
                }
                ply:BRS():AddInventoryItem(itemData, (reqInfo[2] or 1)) 
            end
