	...
	if( sameAsRequired ) then
		...
		if( currencyTable and currencyTable.addFunction ) then
			...
			if( currencyTable.formatFunction ) then 
				DarkRP.notify( ply, 1, 5, "Вы получили " .. currencyTable.formatFunction( amount*(sameAsRequired.Price or 0) ) .. " от торговли!" )
			else
				DarkRP.notify( ply, 1, 5, "Вы получили " .. string.Comma( amount*(sameAsRequired.Price or 0) ) .. " " .. NPCTable.ReqInfo[1] .. " от торговли!" )
			end
		else
		...
		if( (currencyTable.getFunction( ply ) or 0) >= price ) then
			...
			if( typeTable.SellingTypes and typeTable.SellingTypes[itemTable.Type] and typeTable.SellingTypes[itemTable.Type].GiveItem ) then
				...
				if( giveItem != false ) then
					...
					if( currencyTable.formatFunction ) then 
						DarkRP.notify( ply, 1, 5, "Вы купили x" .. amount .. " " .. (typeTable.SellingTypes[itemTable.Type].FormatName( itemTable.ReqInfo or {} ) or "item") .. " за " .. currencyTable.formatFunction( price ) .. " у Торговца!" )
					else
						DarkRP.notify( ply, 1, 5, "Вы купили x" .. amount .. " " .. (typeTable.SellingTypes[itemTable.Type].FormatName( itemTable.ReqInfo or {} ) or "item") .. " за " .. string.Comma( price ) .. " " .. NPCTable.ReqInfo[1] .. " у Торговца!" )
					end
				else
