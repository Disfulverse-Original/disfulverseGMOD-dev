...
function CheckPlayersAllDead(k, attacker)
    if not BRICKS_SERVER.TEMP.BOSS_DAMAGE or not attacker then return false end
    --PrintTable(BRICKS_SERVER.TEMP.BOSS_DAMAGE[bossEntity])
    for player, _ in pairs(BRICKS_SERVER.TEMP.BOSS_DAMAGE[attacker]) do

    	--if player == k then BRICKS_SERVER.TEMP.BOSS_DAMAGE[attacker][player] = nil PrintTable(BRICKS_SERVER.TEMP.BOSS_DAMAGE[attacker]) end

    	--print(tostring(IsValid(player)) .. tostring(player:Alive()) )

        if IsValid(player) and player:Alive() then
            return false  -- At least one player is still alive
        end
    end
    --print("dead on func check")
...
hook.Add( "OnNPCKilled", "BRS.OnNPCKilled_Boss", function( npc, attacker, inflictor )
	if( npc:GetNW2Int( "BRICKS_SERVER_BOSS_KEY" ) > 0 and BRICKS_SERVER.CONFIG.BOSS.NPCs[npc:GetNW2Int( "BRICKS_SERVER_BOSS_KEY" )] ) then
		...
		if( BRICKS_SERVER.TEMP.BOSS_DAMAGE and BRICKS_SERVER.TEMP.BOSS_DAMAGE[npc] ) then
			...
			--PrintTable(BRICKS_SERVER.TEMP.BOSS_DAMAGE[npc])
		end
        ...
        --if CheckPlayersAllDead(npc) then
        	--print("hp set for boss!")
            --npc:SetHealth(BRICKS_SERVER.CONFIG.BOSS.NPCs[npc:GetNW2Int("BRICKS_SERVER_BOSS_KEY")].Health)
        --end
...
hook.Add("PlayerDeath", "BRS.PlayerDeath_BossDamage", function(victim, inflictor, attacker)
    -- Check if the victim was associated with any boss entity
	--BRICKS_SERVER.TEMP.BOSS_DAMAGE[attacker] = nil
    ...
    if BRICKS_SERVER.TEMP.BOSS_DAMAGE and BRICKS_SERVER.TEMP.BOSS_DAMAGE[attacker] then
        ...
        for k, damageTable in pairs(BRICKS_SERVER.TEMP.BOSS_DAMAGE[attacker]) do
        	--print(k)
            if k == victim then
            	BRICKS_SERVER.TEMP.BOSS_DAMAGE[attacker][k] = nil
                --print("Player removed from boss damage table")
                --print( ( table.ToString(BRICKS_SERVER.TEMP.BOSS_DAMAGE[attacker]) ) or "no players found" )
                -- Check if all associated players with this boss are dead
                if CheckPlayersAllDead(k, attacker) then
                    -- Reset boss health to max
                    local bossKey = attacker:GetNW2Int("BRICKS_SERVER_BOSS_KEY", 0)
                    if bossKey > 0 and BRICKS_SERVER.CONFIG.BOSS.NPCs[bossKey] then
                    	--print("true")
                        attacker:SetHealth(BRICKS_SERVER.CONFIG.BOSS.NPCs[bossKey].Health)
                    end
                end
            end
        end
