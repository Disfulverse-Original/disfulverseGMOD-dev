...
function BRICKS_SERVER.Func.CreateCraftingEditor( oldCraftingTable, onSave, onCancel )
	...
	function backLeftPanel.RefreshInfo()
		...
		local crafttoolsListBack = vgui.Create( "bricks_server_scrollpanel", itemInfoDisplay )
		crafttoolsListBack:Dock( RIGHT )
		crafttoolsListBack:SetWide( 50 )
		crafttoolsListBack:DockMargin( 0, 75, 25, 0 )
		crafttoolsListBack.Paint = function( self2, w, h ) end

		for k, v in pairs( craftingTable.CraftTools ) do
			local modelEntryButton = vgui.Create( "DPanel", crafttoolsListBack )
			modelEntryButton:Dock( TOP )
			modelEntryButton:SetTall( crafttoolsListBack:GetWide() )
			modelEntryButton:DockMargin( 0, 0, 0, 5 )
			local changeAlpha = 0
			local modelEntryIcon
			local x, y, w, h = 0, 0, modelEntryButton:GetTall(), modelEntryButton:GetTall()
		...
		for k, v in pairs( craftingTable.Blueprints ) do
	...
	local actions = {
		...
		[4] = { "Tools required", Material( "materials/bricks_server/log.png" ), false },
		[5] = { "Blueprints required", Material( "materials/bricks_server/log.png" ), false },
		[6] = { "Model", Material( "materials/bricks_server/icon.png" ), function()
		...
		[7] = { "Model Color", Material( "materials/bricks_server/color.png" ), function()
		...
		[8] = { "Craft time", Material( "materials/bricks_server/time.png" ), function()
		...
		[9] = { "Group", Material( "materials/bricks_server/group.png" ), function()
			local options = {}
			options["None"] = "None"
	...
	if( BRICKS_SERVER.Func.IsSubModuleEnabled( "essentials", "levelling" ) ) then
		actions[10] = { "Level", Material( "materials/bricks_server/level.png" ), function()
	...
	function backPanel.FillOptions()
		...
		for k, v in ipairs( actions ) do
			...
			elseif( k == 4 ) then
				...
				for k, v in pairs( BS_ConfigCopyTable.CRAFTING.CraftTools or {} ) do
					if( ((craftingTable.CraftTools or {})[k] or 0) ~= true ) then continue end
					...
					modelEntryIcon.DoClick = function()
						BRICKS_SERVER.Func.Query( "Удалить ли этот инструмент?", "Admin", "Да", "Нет", function()

							craftingTable.CraftTools[k] = nil
				...
				modelEntryButton.DoClick = function()
					local newCraftTools = {}
					for k, v in pairs( BS_ConfigCopyTable.CRAFTING.CraftTools or {} ) do
						--PrintTable(BS_ConfigCopyTable.CRAFTING.CraftTools)
						--PrintTable(craftingTable.CraftTools[k])
						if( ((craftingTable.CraftTools or {})[k] or 0) == true ) then continue end

						newCraftTools[k] = k
					end

					BRICKS_SERVER.Func.ComboRequest( "Admin", "What tool would you like to add?", "", newCraftTools, function( value, data ) 
						if( newCraftTools[value] ) then
							craftingTable.CraftTools = craftingTable.CraftTools or {}
							craftingTable.CraftTools[value] = true
							--PrintTable(craftingTable)

							backPanel.FillOptions()
							backLeftPanel.RefreshInfo()
						else
							notification.AddLegacy( "Invalid tool.", 1, 3 )
						end
					end, function() end, "OK", "Cancel", true )
				end
			elseif( k == 5 ) then
				...
				for k, v in pairs( BS_ConfigCopyTable.CRAFTING.Blueprints or {} ) do
					if( ((craftingTable.Blueprints or {})[k] or 0) ~= true ) then continue end
					--PrintTable(BS_ConfigCopyTable.CRAFTING.Blueprints)
					backPanel.grid.slots = (backPanel.grid.slots or 0)+1
					local slots = backPanel.grid.slots
					local slotsTall = math.ceil( slots/slotsWide )
					backPanel.grid:SetTall( (slotsTall*slotSize)+((slotsTall-1)*spacing) )
					...
					modelEntryIcon.DoClick = function()
						BRICKS_SERVER.Func.Query( "Удалить ли эту схему?", "Admin", "Да", "Нет", function()

							craftingTable.Blueprints[k] = nil
				...
				modelEntryButton.DoClick = function()
					...
					for k, v in pairs( BS_ConfigCopyTable.CRAFTING.Blueprints or {} ) do
						--PrintTable(BS_ConfigCopyTable.CRAFTING.Blueprints)
						--PrintTable(craftingTable.Blueprints[k])
						if( ((craftingTable.Blueprints or {})[k] or 0) == true ) then continue end
						--print(namae)
						newBlueprints[k] = k
						--PrintTable(newBlueprints)
					end

					BRICKS_SERVER.Func.ComboRequest( "Admin", "What tool would you like to add?", "", newBlueprints, function( value, data ) 
						if( newBlueprints[value] ) then
							craftingTable.Blueprints = craftingTable.Blueprints or {}
							craftingTable.Blueprints[value] = true
							--PrintTable(craftingTable)
		...
		if( backPanel.grid and IsValid( backPanel.grid ) ) then
			backgroundPanel:SetTall( buttonPanel:GetTall()+(3*10)+(#actions*55)+(#itemReqInfo*90)+backPanel.grid:GetTall() )
		else
			backgroundPanel:SetTall( buttonPanel:GetTall()+(3*10)+(#actions*55)+(#itemReqInfo*90) )
		end
		backgroundPanel:Center()
