...
function playerMeta:TakeBlueprintUse(blueprint)
    local plyInventory = self:BRS():GetInventory()
    local blueprintNeeded = table.Copy(blueprint)

    for k, v in pairs(plyInventory) do
        if v[2][1] == "bricks_server_blueprint" and blueprintNeeded[v[2][4]] then
            local blueprintUseCurr = plyInventory[k][2][6]

            -- Subtract from the current use amount of this blueprint
            plyInventory[k][2][6] = blueprintUseCurr - 1
            --print(plyInventory)
            if plyInventory[k][2][6] <= 0 then
                -- Remove the blueprint from inventory if its use amount reaches zero
                plyInventory[k] = nil
                table.remove(plyInventory, k)
                DarkRP.notify(self, 1, 5, "При создании предмета, схема была полностью исчерпана!")
            end

            break -- Exit the loop after modifying the first suitable blueprint
        end
    end
...
local function craftItem( ply, itemKey )
	...
	if( not BRICKS_SERVER.Func.HasResources( (plyInventory or {}), (craftingTable.Resources or {}) ) ) then
		DarkRP.notify( ply, 1, 5, "Создание " .. craftingTable.Name .. " не удалось, недостаточно ресурсов!" )
	elseif( not BRICKS_SERVER.Func.HasTools( (plyInventory or {}), (craftingTable.CraftTools or {}) ) ) then
		DarkRP.notify( ply, 1, 5, "Создание " .. craftingTable.Name .. " не удалось, нет нужных инструментов!" )
	elseif( not BRICKS_SERVER.Func.HasBlueprint( (plyInventory or {}), (craftingTable.Blueprints or {})  ) ) then
		DarkRP.notify( ply, 1, 5, "Создание " .. craftingTable.Name .. " не удалось, нет нужной схемы!" )
	else
		DarkRP.notify( ply, 1, 5, "Создание " .. craftingTable.Name .. " завершено!" )

		ply:TakeResources( (craftingTable.Resources or {}) )
		ply:TakeBlueprintUse( (craftingTable.Blueprints or {}) )
...
net.Receive( "BRS.Net.CraftItem", function( len, ply )
	...
	if( BRICKS_SERVER.Func.IsSubModuleEnabled( "essentials", "levelling" ) and craftingTable.Level ) then
		if( ply:GetLevel() < craftingTable.Level ) then
			DarkRP.notify( ply, 1, 5, "Ваш уровень не соответствует требуемому для создания этого предмета!" )
			return
		end
	end
	...
	if( craftingTable.Group ) then
		if( not BRICKS_SERVER.Func.IsInGroup( ply, craftingTable.Group ) ) then
			DarkRP.notify( ply, 1, 5, "Ваша группа не подходит для создания этого предмета!" )
			return
		end
	end
	...
	if( not BRICKS_SERVER.Func.HasResources( (plyInventory or {}), (craftingTable.Resources or {}) ) ) then
		DarkRP.notify( ply, 1, 5, "У вас недостаточно ресурсов для создания этого предмета!" )
		return
	end

	if( not BRICKS_SERVER.Func.HasTools( (plyInventory or {}), (craftingTable.CraftTools or {}) ) ) then
		--PrintTable(craftingTable.CraftTools)
		DarkRP.notify( ply, 1, 5, "У вас нет нужных инструментов для этого!" )
		return
	end

	if( not BRICKS_SERVER.Func.HasBlueprint( (plyInventory or {}), (craftingTable.Blueprints or {}) ) ) then
		--PrintTable(craftingTable.Blueprints)
		DarkRP.notify( ply, 1, 5, "У вас нет необходимой схемы для этого!" )
		return
	end
...
net.Receive( "BRS.Net.CraftCancel", function( len, ply )
	...
	if( craftingTable ) then
		DarkRP.notify( ply, 1, 5, "Создание " .. craftingTable.Name .. " отменено!" )
	end
