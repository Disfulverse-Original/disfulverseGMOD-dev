...
function PANEL:FillPanel()
    ...
    function self.RefreshItems()
        ...
        local itemsOrdered = {}
        for k, v in pairs( BRICKS_SERVER.CONFIG.DEATHSCREENS.Soundtracks ) do
        ...
        for k, v in pairs( itemsOrdered ) do
            local itemKey = v[1]
            local itemTable = BRICKS_SERVER.CONFIG.DEATHSCREENS.Soundtracks[itemKey]
            ...
            itemBack.Paint = function( self2, w, h )
                ...
                if( BRS_DEATHSCREENS_DATA and BRS_DEATHSCREENS_DATA[3] and BRS_DEATHSCREENS_DATA[3][itemKey] ) then
                    if( BRS_DEATHSCREENS_DATA[3][itemKey][1] == true ) then
                ...
                local textX, textY = surface.GetTextSize( itemTable.Name or "New Soundtrack" )
                local boxW, boxH = textX+10, textY

                draw.RoundedBox( 5, 15, 15, boxW, boxH, (itemTable[2] or BRICKS_SERVER.Func.GetTheme( 5 )) )
                draw.SimpleText( itemTable.Name or "New Soundtrack", "BRICKS_SERVER_Font25", 15+(boxW/2), 15+(boxH/2)-1, BRICKS_SERVER.Func.GetTheme( 6 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
            end
            ...
            purchaseButton.Paint = function( self3, w, h )
                ...
                if( BRS_DEATHSCREENS_DATA and BRS_DEATHSCREENS_DATA[3] and BRS_DEATHSCREENS_DATA[3][itemKey] ) then
                    draw.SimpleText( "Set active", "BRICKS_SERVER_Font33", w/2, (h/2)-1, BRICKS_SERVER.Func.GetTheme( 6 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
                else
                    draw.SimpleText( "Unlock", "BRICKS_SERVER_Font33", w/2, (h/2)-1, BRICKS_SERVER.Func.GetTheme( 6 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
                end
                surface.SetAlphaMultiplier( 1 )
            end
            ...
            local playMat = Material( "materials/bricks_server/play.png" )
            local pauseMat = Material( "materials/bricks_server/pause.png" )
            local button = vgui.Create( "DButton", itemBack )
            button:SetSize( 36, 36 )
            button:SetPos( (emblemSlotSize/2)-(button:GetWide()/2), (slotTall/2)-(button:GetTall()/2) )
            button:SetText( "" )
            local changeAlpha = 0
            button.Paint = function( self3, w, h )
                ...
                if( not self.testSound or (self.testSoundItem or "") != itemKey ) then
                ...
                if( not self.testSound or (self.testSoundItem or "") != itemKey ) then
                    ...
                    sound.PlayURL( (itemTable.Sound or ""), "mono", function( station )
                        if( IsValid( station ) ) then
                            self.testSound = station
                            self.testSound:Play()
                            self.testSoundItem = itemKey
...
vgui.Register( "bricks_server_deathscreens_soundtracks", PANEL, "DPanel" )