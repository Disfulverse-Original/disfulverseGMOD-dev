...
--Открывает инвентарь в контекст меню
		...
		if( IsValid( ent ) ) then
			...
			if( (BRICKS_SERVER.CONFIG.INVENTORY.Whitelist or {})[entClass] ) then
				...
				if ent:GetClass() == "spawned_weapon" then
					pickupText = "Нажмите, " .. string.upper( (BRICKS_SERVER.Func.GetClientBind( "PickupBind2" ) or "UNBOUND") ) .. " чтобы взять в руки"
				elseif( binds[1] and binds[2] ) then
