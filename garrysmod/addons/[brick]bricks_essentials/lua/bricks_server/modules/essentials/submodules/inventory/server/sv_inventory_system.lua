...
function BRICKS_SERVER.PLAYERMETA:AddInventoryEnt( itemEntity )
	...
	--PrintTable(itemData)
	--print(itemData[2])
	--print(itemEntity:GetOwner())

	if itemData[1] == "bricks_server_resource" then
		if( self:IsInventoryFull( amount, true ) ) then 
	...
	DarkRP.notify( self.Player, 1, 5, "Добавлено предметов: " .. amount .. " в инвентарь!" )
	itemEntity:Remove()
end
...
hook.Add( "PlayerButtonDown", "BRS.PlayerButtonDown_InventoryPickup", function( ply, key )
	...
	if( ply:GetEyeTrace() and ply:GetEyeTrace().Entity and IsValid( ply:GetEyeTrace().Entity ) ) then
		...
		--PrintTable(BRICKS_SERVER.Func.GetEntTypeField( ply:GetEyeTrace().Entity, "GetItemData" ))
...
net.Receive( "BRS.Net.InventoryDropItem", function( len, ply )
	...
	if( inventoryTable[itemKey] ) then
		...
		if( not preventDrop ) then
			...
			DarkRP.notify( ply, 1, 5, "Выброшено предметов: "..itemTable[1] .. " из вашего инвентаря." )
		else
...
function BRICKS_SERVER.Func.HolsterWeapon( ply )
	...
	local CurrentClip = activeWeapon:Clip1() or 0
	--local maxClipSize = activeWeapon:GetMaxClip1()

	local ReserveAll = ply:GetAmmoCount(activeWeapon:GetPrimaryAmmoType())
	local ReserveNotAll = ReserveAll - (CurrentClip or 0)

	if ReserveNotAll < 0 then ReserveClip = ReserveAll
	else ReserveClip = ReserveNotAll end

	--17 - 30 = -13
	--		     17

	--print("Reserve clip before insert is " .. " " .. ReserveClip)
	local itemData = { "spawned_weapon", (activeWeapon.WorldModel or BRICKS_SERVER.Func.GetWeaponModel( weaponClass )), weaponClass }
	table.insert( itemData, 6, CurrentClip or 0 )
	table.insert( itemData, 7, ReserveClip or 0 )
	--PrintTable(itemData)
	--[[
	if( BRICKS_SERVER.Func.IsSubModuleEnabled( "essentials", "swepupgrader" ) ) then
	...
	--]]
	DarkRP.notify( ply, 1, 1.5, "Убираем " .. activeWeapon:GetPrintName() .. " в инвентарь." )

	local delaynum = 1.5

	timer.Simple(delaynum, function()

		local beforeammo = ply:GetAmmoCount(activeWeapon:GetPrimaryAmmoType())

		ply:StripWeapon( weaponClass )
		ply:SetAmmo(beforeammo-ReserveClip, activeWeapon:GetPrimaryAmmoType())

		--print("before ammo is ".. beforeammo .. " " .. "ammo type is " .. activeWeapon:GetPrimaryAmmoType() .. " " .. "Eventually ammo: ".. beforeammo-ReserveClip)
...
function BRICKS_SERVER.Func.RecalcInventory(ply)
    if not IsValid(ply) then return end

    local invslots = BRICKS_SERVER.Func.GetInventorySlots(ply)
    local plyinv = ply:BRS():GetInventory()
    --PrintTable(plyinv)
    -- Count the number of items currently in the player's inventory
    local itemCount = table.Count(plyinv)


    -- Check if the number of items exceeds the available inventory slots
    if itemCount >= invslots then
        -- Sort items by their slot indices (assuming items have a 'slot' field)
	    table.sort(plyinv, function(a, b)
	        return (plyinv[a] or 0) < (plyinv[b] or 0) -- Sort items by slot index
	    end)
        -- Create a new inventory table to hold items that fit within available slots
        local newInventory = {}
        local currentSlot = 1

        for _, item in pairs(plyinv) do
            if currentSlot <= invslots then
                table.insert(newInventory, item)
                currentSlot = currentSlot + 1
            else
                -- Remove excess items that exceed available slots
                -- You may want to perform additional logic here (e.g., dropping items, notifying the player)
                -- For demonstration purposes, we simply skip adding items beyond available slots
            end
        end

        -- Update the player's inventory to the new sorted and trimmed inventory
        --PrintTable(newInventory)
        ply:BRS():SetInventory(newInventory)
    end
end

--[[
for i, v in ipairs( player.GetAll() ) do
    BRICKS_SERVER.Func.RecalcInventory(v)
end
--]]
