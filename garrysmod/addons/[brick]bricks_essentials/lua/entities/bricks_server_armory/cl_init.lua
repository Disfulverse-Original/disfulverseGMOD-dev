...
function ENT:Draw()
	...
	cam.Start3D2D( self:GetPos()+(selfAngles:Up()*7)-(selfAngles:Right()*95), selfAngles, 0.1 )
		...
		draw.SimpleText( "Оружейный шкаф", "BRICKS_SERVER_Font30", x+(w/2), y, BRICKS_SERVER.Func.GetTheme( 6 ), TEXT_ALIGN_CENTER, 0 )
		...
		draw.SimpleText( "Полиция", "BRICKS_SERVER_Font17", x+textXSpacing, textYPos-2, BRICKS_SERVER.Func.GetTheme( 5 ), TEXT_ALIGN_CENTER, 0 )
		...
		draw.SimpleText( "Хранит", "BRICKS_SERVER_Font17", x+w-textXSpacing, textYPos-2, BRICKS_SERVER.Func.GetTheme( 5 ), TEXT_ALIGN_CENTER, 0 )
	cam.End3D2D()
