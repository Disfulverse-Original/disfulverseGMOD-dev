...
function RCD.CreateNPC(npcId, name, model, pos, ang, plateforms, groups, map)
    ...
    if edit then
        RCD.Query(("UPDATE rcd_npc SET map = %s, name = %s, model = %s, pos = %s, ang = %s, plateforms = %s, groups = %s WHERE map = %s AND id = %s"):format(RCD.Escape(map), RCD.Escape(name), RCD.Escape(model), RCD.Escape(pos), RCD.Escape(ang), RCD.Escape(plateforms), RCD.Escape(groups), RCD.Escape(RCDMap), RCD.Escape(npcId)), function()
            RCD.CreateNPCEntity(pos, ang, model, npcId)
        end)
    else
        RCD.Query(("INSERT INTO rcd_npc (map, name, model, pos, ang, plateforms, groups) VALUES (%s, %s, %s, %s, %s, %s, %s)"):format(RCD.Escape(map), RCD.Escape(name), RCD.Escape(model), RCD.Escape(pos), RCD.Escape(ang), RCD.Escape(plateforms), RCD.Escape(groups)), function(tbl)
...
--[[ Create NPC entity ]]
function RCD.CreateNPCEntity(pos, ang, model, npcId)
...
function RCD.SetNPCParams(npcId, npc)
    ...
    RCD.Query(("SELECT * FROM rcd_npc WHERE map = %s AND id = %s"):format(RCD.Escape(RCDMap), RCD.Escape(npcId)), function(npcTable)
        ...
        npcTable["groups"] = util.JSONToTable(npcTable["groups"] or "") or {}
...
function RCD.RemoveVehicleGroup(id, noCompress, ply)
    ...
    RCD.Query(("SELECT * FROM rcd_npc WHERE map = %s"):format(RCD.Escape(RCDMap)), function(npcTable)
        ...
        for k,v in ipairs(npcTable) do
            ...
            RCD.Query(("UPDATE rcd_npc SET groups = %s WHERE id = %s"):format(RCD.Escape(groupsJson), RCD.Escape(v.id)))
        end
...
function RCD.ManageVehicle(vehicleId, name, price, class, options, groupId, noCompress, ply, callback)
    if not RCD.VehiclesList[class] then
        if isfunction(callback) then
            callback(nil)
        end
