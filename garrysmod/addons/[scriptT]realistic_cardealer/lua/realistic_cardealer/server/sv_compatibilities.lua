...
function RCD.TransfertWCD(ply)
    ...
    for k, npcData in ipairs(npcsList) do
        ...
        RCD.CreateNPC(nil, npcData.name, npcData.model, npcData.pos, npcData.ang, (npcData.platforms or {}), groups, RCDMap)
    ...
    if WCD.Storage.type == "sqllite" then
        ...
        for k, data in ipairs(playerData) do
            data["value"] = util.JSONToTable(data["value"]) or {}
        ...
        for k, v in pairs(vehiclesToSave) do
            ...
            v.customization = util.TableToJSON(v.customization) or ""
    ...
    elseif WCD.Storage.type == "mysqloo9" then
        ...
        function query:onSuccess(data)
            for k, v in ipairs(data) do
                ...
                v.owned = util.JSONToTable(v.owned) or {}
...
function RCD.TransfertVCMOD(ply)
    ...
    for fileId, fileTxt in pairs((files or {})) do
        local npcTable = util.JSONToTable(file.Read(("vcmod/cardealer/maps/%s/%s"):format(RCDMap, fileTxt), "DATA")) or {}
        ...
        RCD.CreateNPC(nil, npcTable["Name"], npcTable["Model"], npcTable["Pos"], npcTable["Ang"], plateforms, {[groupId] = true}, RCDMap)
    ...
    for fileId, fileTxt in pairs((files or {})) do
        local playerTable = util.JSONToTable(file.Read(("vcmod/cardealer/plydata/%s"):format(fileTxt), "DATA")) or {}
        ...
        for vehicleId, customization in pairs(vehiclesList) do
            ...
            saveCustomization = util.TableToJSON(saveCustomization) 
...
function PLAYER:RCDGiveVehiclesCompatibilities()
    ...
    RCD.Query(("SELECT * FROM rcd_bough_compatibilities WHERE playerId = %s"):format(RCD.Escape(uniqueId)), function(vehiclesTable)
        ...
        for k,v in ipairs(vehiclesTable) do
            v.vehicleId = tonumber(v.vehicleId)
            v.customization = util.JSONToTable(v.customization) or {}
...
function RCD.TransfertAdvancedCarDealer(ply)
    ...
    for k,v in pairs(configDatas["Vehicles"]) do
        for brandKey, vehicles in pairs(v) do
            ...
            local success, vehicleTable = RCD.ManageVehicle(nil, vehicles.name, (v.priceCatalog or 1000), vehicles.className, optionsTable, groupId, true, nil, "wait")
            if not success then continue end
            local rcdId = tonumber(vehicleTable.id)
...
function RCD.TransfertModernCarDealer(ply)
    ...
    for k,v in pairs((npcDatas["Dealers"] or {})) do
        ...
        RCD.CreateNPC(nil, k, v.Model, v.Position, v.Angles, {}, {[groupId] = true}, RCDMap)
...
hook.Add("VC_canSwitchSeat", "RCD:Compatibilities:VC_canSwitchSeat", function(ply, ent_from, ent_to)
    ...
    if securityBelt then ply:RCDNotification(5, RCD.GetSentence("cantSwitch")) end
