...
function PLAYER:RCDManageJobVehicle(showcase, vehicleParams)
    ...
    if not istable(showcase.RCDVehicle) then
        ...
        if self:RCDGetMoney() < rentPrice then self:RCDNotification(5, RCD.GetSentence("notEnoughtMoney")) return end
...
hook.Add("PhysgunPickup", "RCD:PhysgunPickup:Protect", function(ply, ent)
    if ent:GetClass() == "rcd_showcase" && istable(ent.RCDVehicle) then
        return false
    end
end)
...
function PLAYER:RCDStartPrinting(ent, vehicleId, vehicleParams)
	...
	ent.RCDCount = ent.RCDCount or 0
	if ent.RCDCount >= 1 then self:RCDNotification(5, self:RCDNotification(5, RCD.GetSentence("invoiceLimit"))) return end
