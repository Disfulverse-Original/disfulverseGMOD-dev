...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < self:GetNWInt("distance") then
		cam.Start3D2D(pos+ang:Up()*3.35, ang, 0.07)
			draw.SimpleText("Жидая", "methFont", 0, -14, sulfurColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
			draw.SimpleText("сера", "methFont", 0, 10, sulfurColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
			draw.SimpleText(""..self:GetNWInt("amount").."л", "methFont", 0, 34, sulfurColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
		cam.End3D2D();
