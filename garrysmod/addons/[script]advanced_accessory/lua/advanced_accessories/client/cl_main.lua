...
function AAS.UpdateList(tbl, bool)
    if IsValid(accessoriesFrame) and IsValid(itemContainer) and IsValid(itemScroll) then
        itemContainer:Clear()

        if #tbl == 0 and IsValid(sliderList) then AAS.ItemMenu() end
        for k,v in ipairs(tbl) do
            ...
            if v.options["vip"] or isNew then
                if (not v.options["vip"] and isNew) and not AAS.ClientTable["filters"]["new"] then continue end
                if (v.options["vip"] and not isNew) and not AAS.ClientTable["filters"]["vip"] then continue end
                if (v.options["vip"] and isNew) and (not AAS.ClientTable["filters"]["new"] and not AAS.ClientTable["filters"]["vip"]) then continue end
            end
            ...
            if IsValid(sliderList) then
                for k,v in pairs(AAS.ClientTable["ItemsVisible"]) do
...
function AAS.CreateClientAccessory(uniqueId, steamId, offsetTable)
...
net.Receive("AAS:Main", function()
    ...
    if UInt == 1 then
        ...
        AAS.ClientTable["ItemsTable"] = table.ClearKeys(AAS.ClientTable["ItemsTable"])
        
        table.sort(AAS.ClientTable["ItemsTable"], function(a, b)
    ...
    elseif UInt == 8 then
        ...
        local customOffset = AAS.UnCompressTable(data) or {}

        AAS.ClientTable["ItemsOffsetModel"] = customOffset
    end 
end)
...
net.Receive("AAS:Inventory", function()
    ...
    elseif UInt == 2 then
        ...
        AAS.CreateClientAccessory(uniqueId, steamId, offsetTable)

        local itemTable = AAS.GetTableById(uniqueId)
        
        if AAS.ModifyOffset and IsValid(accessoriesFrame) and steamId == LocalPlayer():SteamID64() then
    ...
    elseif UInt == 5 then
        ...
        for k,v in ipairs(player.GetAll()) do
            ...
            for category, uniqueId in pairs(tbl[v:SteamID()]) do
                ...
                AAS.CreateClientAccessory(uniqueId, v:SteamID64(), tbl)
            end
        end
    elseif UInt == 6 then
...
function AAS.BaseMenu(title, info, sizeX, icon, text)
    ...
    accessoriesFrame.Paint = function(self,w,h)
        ...
        if #(AAS.ClientTable["ItemsVisible"] or {}) == 0 and text then
...
function AAS.ItemMenu()
    ...
    searchBar:SetPos(accessoriesFrame:GetWide()*0.085, AAS.ScrH*0.055)
    ...
    vipButton:SetPos(accessoriesFrame:GetWide()*0.385, AAS.ScrH*0.055)
...
hook.Add("PostPlayerDraw" , "AAS:PostPlayerDraw", function(ply)
    ...
    if AAS.CantHaveAccessoryDraw[team.GetName(ply:Team())] then return end
    ...
    for k,v in pairs(AAS.ClientTable["ItemsEquiped"][ply:SteamID64()]) do
        ...
        if not isnumber(ply:LookupBone(itemTable.options.bone)) then
            for i=1, ply:GetBoneCount() do
                local name = ply:GetBoneName(i)
                
                AAS.SimilarityBones[itemTable.options.bone] = AAS.SimilarityBones[itemTable.options.bone] or {}
                local bool = AAS.SimilarityBones[itemTable.options.bone][name]
        ...
        if not isnumber(boneid) then continue end
