...
function PANEL:AddItemView(itemScroll, accessoriesFrame, itemContainer, tbl)
    ...
    item.Paint = function(panel,w,h)
        ...
        if istable(self.AASTable["options"]) and self.AASTable["options"]["vip"] then 
            surface.SetDrawColor(AAS.Colors["black18"])
            --surface.SetMaterial(AAS.Materials["vip"])
            surface.DrawRect(0, AAS.ScrH*0.17, w, h*0.1)

            surface.SetFont( "AAS:Font:03" )
            surface.SetTextColor(AAS.Colors["white"])
            surface.SetTextPos( w*0.175, AAS.ScrH*0.17 ) 
            surface.DrawText( "DISFULVERSED" )
