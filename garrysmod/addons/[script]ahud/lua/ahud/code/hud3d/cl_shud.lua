...
local function drawPly(ply)
    ...
    if !ahud.DisableModules.Overhead and ( l:GetPos():DistToSqr( ply:GetPos() ) < 300 * 300 ) and eyetrace.Entity == ply then
        ...
        cam.Start3D2D( pos, angHead, 0.05 )
            local offsetH = 0
            local colred = Color(210,48,48)
            if DarkRP then
                offsetH = drawMiddleOutlined(ply:GetName(), "ahud_top", offsetH, ply:IsSpeaking() and ura_clr or color_white, false)
                offsetH = drawMiddleOutlined(team.GetName(ply:Team()), "ahud_top", offsetH, team.GetColor(ply:Team()), falsealse) + offsetH
                ...
                elseif ply:getDarkRPVar("AFK") then
                    drawMiddleOutlined("AFK", "ahud_top", offsecolredhite)
                end
            else
