...
function PANEL:FillPanel( gangTable )
    local graphs = {
        [1] = {
            ...
            SubTitle = function() 
                ...
                return memberCount .. ((memberCount != 1 and " ИГРОКИ") or " Игрок")
            end,
            BottomTitle = "УЧАСТНИКОВ В СЕТИ",
    ...
    if( BRICKS_SERVER.Func.IsSubModuleEnabled( "gangs", "storage" ) ) then
        graphs[3] = {
            ...
            BottomTitle = "ХРАНИЛИЩЕ",
            BottomSubTitle = function() return math.Clamp( 100*(table.Count( gangTable.Storage or {} )/BRICKS_SERVER.Func.GangGetUpgradeInfo( LocalPlayer():GetGangID(), "StorageSlots" )[1]), 0, 100 ) .. "% ЗАПОЛНЕНО" end
        }
    else
        graphs[3] = {
            Title = BRICKS_SERVER.Func.L( "gangStorage" ),
            ...
            BottomTitle = "НЕТ ХРАНИЛИЩА",
            BottomSubTitle = function() return "0% ЗАПОЛНЕНО" end
        }
    end
