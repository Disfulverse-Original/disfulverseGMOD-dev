...
function CH_ATM.CanLockpickATM( ply, ent, trace )
	if ent:GetClass() == "ch_atm" then
		...
		-- Player cooldown
		if ply.CH_ATM_LockpickCooldown and ply.CH_ATM_LockpickCooldown > CurTime() then
			CH_ATM.NotifyPlayer( ply, CH_ATM.LangString( "Please wait" ) .." ".. string.ToMinutesSeconds( math.Round( ply.CH_ATM_LockpickCooldown - CurTime() ) ) .. " ".. CH_ATM.LangString( "before lockpicking another ATM." ) )
			return false
		end
...
function CH_ATM.StartLockpickATM( ply, ent, trace )
	if ent:GetClass() == "ch_atm" then
		...
		-- Make player wanted
		if CH_ATM.MakePlayerWantedOnHack then
	...
	if is_hacking then
		if attacker and attacker:IsPlayer() and IsValid( attacker ) then
			...
			if attacker:CH_ATM_IsPoliceJob() then
				...
				CH_ATM.LevelSupport( attacker, amount )
	...
	if is_hacking then
		if arrester and arrester:IsPlayer() and IsValid( arrester ) then
			...
			if arrester:CH_ATM_IsPoliceJob() then
				...
				CH_ATM.LevelSupport( arrester, amount )
