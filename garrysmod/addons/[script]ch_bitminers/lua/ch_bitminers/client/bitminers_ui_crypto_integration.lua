...
net.Receive( "CH_BITMINERS_CryptoOptions", function( length, ply )
	...
	GUI_CryptoOptionFrame:SetSize( ScrW() * 0.36, ScrH() * 0.445 )
	...
	GUI_CryptoOptionFrame.Paint = function( self, w, h )
		...
		-- Draw top
		draw.RoundedBoxEx( 8, 0, 0, w, ScrH() * 0.03, CH_CryptoCurrencies.Colors.DarkGray, true, true, false, false )

		-- Draw the top title.
		draw.SimpleText( CH_CryptoCurrencies.LangString( "Select Crypto" ), "CH_CryptoCurrency_Font_Size8", w / 2, ScrH() * 0.015, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	end
	...
	GUI_CloseMenu:SetPos( ScrW() * 0.345, ScrH() * 0.01 )
	...
	GUI_CryptoList:SetSize( ScrW() * 0.35, ScrW() * 0.225 )
	GUI_CryptoList:SetPos( ScrW() * 0.005, ScrH() * 0.0375 )
