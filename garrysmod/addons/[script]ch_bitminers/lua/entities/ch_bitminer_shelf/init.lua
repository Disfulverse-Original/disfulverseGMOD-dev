	...
	if self:GetBitcoinsMined() > 0 then
		if CH_Bitminers.Config.IntegrateCryptoCurrencies then
			if not CH_CryptoCurrencies then
				DarkRP.notify( ply, 1, 5, CH_Bitminers.Config.Lang["Payout failed! Cryptocurrencies by Crap-Head has been enabled but is not installed!"][CH_Bitminers.Config.Language] )
				return
			end
...
function ENT:WithdrawInCrypto( ply )
	...
	DarkRP.notify( ply, 1, 5, bitcoins .." ".. crypto_name .." ".. CH_Bitminers.Config.Lang["has been withdrawn from the bitminer!"][CH_Bitminers.Config.Language] )
	DarkRP.notify( ply, 1, 5, CH_Bitminers.Config.Lang["They have been added to your crypto wallet."][CH_Bitminers.Config.Language] )
...
function ENT:EjectBitminer( ply )
	...
	if self:GetMinersInstalled() <= 1 then
		DarkRP.notify( ply, 1, 5, CH_Bitminers.Config.Lang["There has to be more than one bitminer in order to eject it!"][CH_Bitminers.Config.Language] )
		return
	end
	...
	DarkRP.notify( ply, 1, 5, CH_Bitminers.Config.Lang["You have ejected a bitminer. It has been spawned next to you."][CH_Bitminers.Config.Language] )
end
