...
local function OpenCart(Panel)
	...
	CartP = vgui.Create( "DScrollPanel", Panel )
	...
	CartP.Paint = function()
		-- ++draw.SimpleText( "Cart", "Bariol100",w/2,30,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
	end
	...
	head.Paint = function()
		draw.SimpleText( CLOTHESMOD.Config.Sentences[17][CLOTHESMOD.Config.Lang].." - "..CLOTHESMOD.Config.Sentences[18][CLOTHESMOD.Config.Lang].." : "..ttprice..CLOTHESMOD.Config.MoneyUnit, "Bariol100",w/2,60/2,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
	end
	
	local nb = 0
	for name,tables in pairs( cart ) do

		local dCart = vgui.Create( "DScrollPanel", CartP )
		dCart:SetSize( w-20, 50 )
		dCart:SetPos( 10,60 + (50+10)*nb )
		dCart.Paint = function(pnl, wi,he)
			draw.RoundedBox(0,0,0,wi,2, Color(255,255,255,255))
			draw.RoundedBox(0,0,he-2,wi,2, Color(255,255,255,255))
			draw.SimpleText( string.sub(name,0,string.len(name)-1).." - "..tables.price..CLOTHESMOD.Config.MoneyUnit, "Bariol100",wi/2,he/2,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, 1 )
		end
		
		ttprice = ttprice + tables.price
		
		local DButton = vgui.Create( "DButton", dCart )
		...
		DButton.Paint = function(p, w, h)
			...
			draw.SimpleText( "X", "Bariol100",w/2,h/2,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, 1 )
...
local function OpenList( Panel, cat, CP )
	...
	DPanel.Paint = function()
		draw.SimpleText( CLOTHESMOD.Config.Sentences[20][CLOTHESMOD.Config.Lang], "Bariol100",w/2,30,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
		if err ~= "" then
			draw.SimpleText( CLOTHESMOD.Config.Sentences[10][CLOTHESMOD.Config.Lang]..": "..err, "Bariol100",w/2,60,Color( 255, 0, 0, 255 ), TEXT_ALIGN_CENTER )
		end
	end
	
	DPanel2 = vgui.Create( "DScrollPanel", Panel )
	...
	if infos.sex == 1 then
		listI = CLOTHESMOD.Male
	else
		listI = CLOTHESMOD.Female
	end
	...
	for k,v in pairs( CLOTHESMOD.ShopTextures ) do
		...
		if cat[CLOTHESMOD.Config.Sentences[24][CLOTHESMOD.Config.Lang]] or cat["Short-sleeved"] then
			list[v.name.."3"] = v
	...
	for name, tables in pairs ( list ) do
		...
		DButton2.Paint = function(pnl, w, h )
			...
			draw.SimpleText( name, "Bariol100",w/2,0,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, 0 )
			draw.RoundedBox(0,0,17,w,2, Color(255,255,255,255))

			draw.SimpleText( tables.price..CLOTHESMOD.Config.MoneyUnit, "Bariol100",w/2,h/2-6,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, 0 )
			draw.SimpleText( CLOTHESMOD.Config.Sentences[21][CLOTHESMOD.Config.Lang], "Bariol100",w/2,h/2+6,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, 0 )
			...
			if tables.iscustom then
				for k, v in pairs( CLOTHESMOD.PlayerTops[steamid]["customs"] ) do
					if v.id == tables.id then
						err = CLOTHESMOD.Config.Sentences[22][CLOTHESMOD.Config.Lang]
						return
					end
				end
			elseif tables.type == "bottom" then
				for k, v in pairs( CLOTHESMOD.PlayerBottoms[steamid] ) do
					if txt ==  k then
						err = CLOTHESMOD.Config.Sentences[22][CLOTHESMOD.Config.Lang]
						return
					end
				...
				for k, v in pairs( CLOTHESMOD.PlayerTops[steamid]["ncustoms"] ) do
					if txt ==  k then
						err = CLOTHESMOD.Config.Sentences[22][CLOTHESMOD.Config.Lang]
						return
					end
...
OpenCharacterPage = function( Frame, sizex, sizey, ent )
	...
	DButton2.Paint = function(pnl, w, h )
		...
		draw.SimpleText( CLOTHESMOD.Config.Sentences[19][CLOTHESMOD.Config.Lang], "Bariol100",(panelx/2-2.5)/2,30/2-1,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	...
	DButton1.Paint = function(pnl, w, h )
		...
		draw.SimpleText( CLOTHESMOD.Config.Sentences[23][CLOTHESMOD.Config.Lang], "Bariol100",(panelx/2-2.5)/2,30/2-1,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	
	end
	DButton1.DoClick = function( pnl )
		DermaPanel:Close()
		net.Start("ClothesMod:BuyClothes")
			net.WriteTable(cart)
			net.WriteEntity(ent)
		net.SendToServer()
	end
	...
	if LocalPlayer():CM_GetInfos().sex == 1 then
		list = CLOTHESMOD.Male
	else
		list = CLOTHESMOD.Female
	end
	
	local categories = {}
	
	categories[CLOTHESMOD.Config.Sentences[24][CLOTHESMOD.Config.Lang]]=true
	...
	DPanelMenu.Paint = function(pnl, w, h)
		...
		for k,v in pairs( categories ) do
			...
			surface.SetFont("Bariol100")
			
			local x = surface.GetTextSize(k)
			if x + 20 + 10 >= 80+50 then
				draw.SimpleText(k, "Bariol100", 20+10,48+30*nbc2+10,Color(255,255,255),0,1)
			else
				draw.SimpleText(k, "Bariol100", 20+10,48+30*nbc2+10,Color(255,255,255),0,1)
			end
			nbc2 = nbc2 + 1
