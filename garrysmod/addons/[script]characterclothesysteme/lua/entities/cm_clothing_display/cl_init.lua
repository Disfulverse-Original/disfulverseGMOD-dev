...
surface.CreateFont( "FontNPCName2", { font = "Cambria", extended  = true, antialias = true, italic = true, blursize = 0, weight = 700, size = 118})
...
function ENT:Draw()
    ...
    if AAS.LocalPlayer:GetPos():DistToSqr(self:GetPos()) < 50000 then
        ...
        draw.DrawText("Продавец одежды", "FontNPCName2", 0, -3085, AAS.Colors["white"], TEXT_ALIGN_CENTER)
        cam.End3D2D()
    end
end