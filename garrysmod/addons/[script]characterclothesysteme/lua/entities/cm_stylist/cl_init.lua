...
function ENT:Draw()
	...
	cam.Start3D2D(position, ang, 0.1)
		...
		local width = draw.SimpleTextOutlined( CLOTHESMOD.Config.Sentences[66][CLOTHESMOD.Config.Lang], "Bariol25" ,0,0, Color(255,255,255,500-dist), 1, 1, 1, Color(0,0,0))
...
local function CreateImageGUI( parent )
	...
	function buttonImg:Think()
		...
		if not IsValid( self ) or not IsValid( dhtml ) then return end
...
local function OpenImage( Panel, Panel2 )
	...
	DLabel:SetText(CLOTHESMOD.Config.Sentences[2][CLOTHESMOD.Config.Lang])
...
local function OpenTee(Panel, Panel2)
	...
	if infos.sex == 1 then
		list = CLOTHESMOD.Male.EditableTop
	else
		list = CLOTHESMOD.Female.EditableTop
	end
...
local function OpenPay( Panel, Panel2 )
	...
	DPanel:SetSize(sizex-4,sizey-4)
	DPanel.Paint = function(pnl,w,h)
		draw.SimpleText( CLOTHESMOD.Config.Sentences[67][CLOTHESMOD.Config.Lang], "Bariol25",w/2,30,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
		...
		draw.SimpleText( CLOTHESMOD.Config.Sentences[56][CLOTHESMOD.Config.Lang].." : "..CLOTHESMOD.Config.EditableTop + nbtxt * CLOTHESMOD.Config.PricePerText + nbimg * CLOTHESMOD.Config.PricePerImg..CLOTHESMOD.Config.MoneyUnit, "Bariol20",w/2,80,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
		if err ~= "" then
	...
	function button:DoClick()
		...
		if not LocalPlayer():canAfford( nbtxt * CLOTHESMOD.Config.PricePerText + nbimg * CLOTHESMOD.Config.PricePerImg ) then
			err = CLOTHESMOD.Config.Sentences[41][CLOTHESMOD.Config.Lang]
			return
		end
	
		MFrame:Remove()
		net.Start("ClothesMod:CustomTeeCreation")
			net.WriteEntity( ent )
...
function ENT:OpenGUI()
	...
	if infos.sex == 1 then
		DPanelDrP:SetSize( sizexm*0.3, sizeym*0.31 )
		DPanelDrP:SetPos( ScrW()/2-sizexm*0.3/2, sizeym*0.23 )
	else
		DPanelDrP:SetSize( sizexm*0.3, sizeym*0.26 )
		DPanelDrP:SetPos( ScrW()/2-sizexm*0.3/2, sizeym*0.24 )
	end
