...
hook.Add("HUDPaintBackground", "darky_ammo_counter", function()

    if DynamicHUD:GetBool() then
        local traceResult = LocalPlayer():GetEyeTrace()
        local arcpos = traceResult.HitPos:ToScreen()

        w, h = math.Round(arcpos.x), math.Round(arcpos.y)
    else
        w, h = ScrW()/2, ScrH()/2
    end
    ...
    if LocalPlayer():IsValid() then
        if not LocalPlayer():InVehicle() or LocalPlayer():InVehicle() and LocalPlayer():GetAllowWeaponsInVehicle() then
            ...
            if Weapon:GetClass() == "weapon_rpt_stungun" then return end
			...
			--]]
                if Weapon:GetMaxClip1() >=1 and DrawAmmo:GetBool() and Weapon:GetPrimaryAmmoType() ~= -1 then
                    ...
                    if Reloading or CurClip <= MaxClip*0.25 then
                        --if IsValid(myarc) then return end
                        myarc()
                        ...
                        if MaxClip>=1 then
                            ...
                            if Reserve>0 then
                                ...
                                if Weapon:GetSecondaryAmmoType()>0 and LocalPlayer():GetAmmoCount(Weapon:GetSecondaryAmmoType())>0 and DrawAmmoOnSingle:GetBool() then
                                    draw.SimpleText("//"..LocalPlayer():GetAmmoCount(Weapon:GetSecondaryAmmoType()), "ARXHUDFONT", w + 40 + Arcd, h + 55 + Arcd, SRcolor, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
                                end
                    ...
                    if ReloadStyle:GetInt() == 1 then
                        AnimVal = math.Round(Lerp(0.08, AnimVal, (Reloading and 1 or 0)), 5)
