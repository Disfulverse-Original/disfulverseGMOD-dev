...
if SERVER then
	...
	local EC_VERSION_WARNING = CreateConVar("easychat_version_warnings", "0", FCVAR_ARCHIVE, "Should we warn users if EasyChat is outdated")
	local WORKSHOP_ID = "3222752892"
...
if CLIENT then
	...
	local _ = CreateConVar("easychat_modern_text_history_limit", "200", FCVAR_ARCHIVE, "Limits how many messages are shown in the modern chat output")
	...
	function EasyChat.Init(ply)
		...
		local function checkplayerif( ply )
			if IsValid(ply) then
				print(ply:Name() .. " from function!")
			else
				print("No Player is found. Player is nil. From function")
			end
		end
		--checkplayerif(LocalPlayer())
		...
		do
			...
			chatbox_frame.BtnSettings:SetVisible(LocalPlayer():IsSuperAdmin())
		...
		function EasyChat.OpenSettings(ply)
