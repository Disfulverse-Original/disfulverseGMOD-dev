...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < self:GetNWInt("distance") then
		...
		cam.Start3D2D(pos+ang:Up()*4.75, ang, 0.09)
				...
				draw.SimpleText(math.Round((self:GetNWInt("amount")*100)/self:GetNWInt("maxAmount")).."% ("..self:GetNWInt("amount").."/"..self:GetNWInt("maxAmount")..")", "methFont1", -170, -7, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT);
