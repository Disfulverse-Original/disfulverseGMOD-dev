...
function ENT:Draw()
	...
	local text2 = ECL.Language.Entities.IsNotSeeded
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < self:GetNWInt("distance") then
		...
		if self:GetNWBool("seeded") then
			...
			time = self:GetNWInt("timer") - CurTime()
			text2 = ECL.Language.Entities.IsSeeded..". "..ECL.Language.Entities.WaitAbout.." "..math.Round(time)..""..ECL.Language.Entities.Sec
