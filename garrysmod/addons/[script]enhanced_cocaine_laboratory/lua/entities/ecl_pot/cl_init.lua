...
function ENT:Draw()
	...
	local text = ECL.Language.Entities.Pot
	local text2 = ECL.Language.Entities.CleanedLeaves..": "..self:GetNWInt("cleaned")
		...
		if self:GetNWInt("max_amount") == self:GetNWInt("cleaned") then
			...
			if self:GetNWInt("timer") < CurTime() then
				...
				if temp < self:GetNWInt("need_temp") then
					text5 = ECL.Language.Entities.NotReady or "Not ready yet";
					cl = Color(255,0,0,alpha)
				else
					text5 = ECL.Language.Entities.Ready or "Ready";
					cl = Color(0,255,0,alpha)
				end
				text4 = ECL.Language.Entities.Temperature..": "..temp.."°";
				...
				if self:GetNWBool("ingnited") then
					...
					elseif self:GetNWInt("temperature") == self:GetNWInt("need_temp") then
						text5 = ECL.Language.Entities.ReadyToUse;
						cl = Color(0,255,0,alpha)
					end
				end
				...
				cam.Start3D2D(Pos+Ang:Right()*-11, Angle(Ang.p, pAng.y-90, Ang.r), 0.075)
					...
					draw.SimpleTextOutlined( text5, "HUDNumber5", -TextWidth5*0.5 + 10, 0, Color(255,255,255,alpha), 0, 0, 1, Color(0,0,0, alpha) )
				cam.End3D2D()
			else
