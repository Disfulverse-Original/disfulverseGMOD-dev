...
function ENT:Draw()
	...
	local text = ECL.Language.Entities.Gas..": "..pers.."%";
	local text2 = ECL.Language.Entities.Plates..":";
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < self:GetNWInt("distance") then
		...
		if (self:GetModel() == "models/srcocainelab/portablestove.mdl") then 
			...
			local text = ECL.Language.Entities.Gas..":";
			...
			cam.Start3D2D(Pos+sAng:Right()*-6.5+sAng:Up()*4.5+sAng:Forward()*8.67, sAng, 0.045)
				...
				draw.SimpleTextOutlined( text, "HUDNumber5", -TextWidth2*0.5 + 10, 0, Color(255,255,255, alpha), 0, 0, 1, Color(0,0,0, alpha) )
			cam.End3D2D()
			...
			if pers > 0 then 
				...
				cam.Start3D2D(Pos+aAng:Right()*8.8+aAng:Up()+aAng:Forward()*-4, aAng, 0.0375)
					draw.SimpleTextOutlined( "Insert the Gascan", "HUDNumber5", 0, 0, Color(255,255,255, alpha), 0, 0, 1, Color(0,0,0, alpha) )
				cam.End3D2D()
			end;
			...
			cam.Start3D2D(Pos+Ang:Right()*-21+Ang:Up()*-19+Ang:Forward()*13, Angle(Ang.p, Ang.y+90, Ang.r-70), 0.06)
				...
				draw.SimpleTextOutlined( text, "HUDNumber5", -TextWidth1*0.5 + 10, 0, Color(255,255,255, alpha), 0, 0, 1, Color(0,0,0, alpha) )
			cam.End3D2D()
