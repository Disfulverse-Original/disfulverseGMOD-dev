...
function ENT:Draw()
	...
	local text = ECL.Language.Entities.CleanedLeaves
	local text2 = ECL.Language.Entities.DraftedLeaves..": "..self:GetNWInt("drafted")
	local text3 = ECL.Language.Entities.SulfuricAcid
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < self:GetNWInt("distance") then
		...
		cam.Start3D2D(Pos+Ang:Right()*-14.5, Angle(Ang.p, pAng.y-90, Ang.r), 0.06)
			...
			draw.SimpleTextOutlined( text3, "HUDNumber5", -TextWidth3*0.5 + 10, 0, Color(255,255,255,alpha), 0, 0, 1, Color(0,0,0, alpha) )
		cam.End3D2D()
