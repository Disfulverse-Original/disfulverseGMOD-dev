...
function NCRequestBox(text, text2, button1, button1fun, button2, button2fun)
	if !IsValid(RequestInput) then 
		...
		RequestInput.Paint = function(self, w, h)
			...
			draw.RoundedBoxEx( 5, 0, 0, w, h, NC.Color.MainPageSecondary, true, true, true, true )	
			draw.SimpleText( text, "RCNPC_InfoText", RequestInput:GetWide()/2, 15, NC.Color.MainPageText, TEXT_ALIGN_CENTER )
			draw.SimpleText( text2, "RCNPC_InfoText", RequestInput:GetWide()/2, 35, NC.Color.MainPageText, TEXT_ALIGN_CENTER )
		end
		
		local Button1 = vgui.Create( "DButton", RequestInput )
		Button1:SetSize( 76, 30 )
		if button2 then
			Button1:SetPos( RequestInput:GetWide()/2-84,RequestInput:GetTall()-45 )
		else
			Button1:SetPos( RequestInput:GetWide()/2-38,RequestInput:GetTall()-45 )
		end
		Button1:SetText( "" )
		...
		if button2 then
			...
			Button2:SetPos( RequestInput:GetWide()/2+8,RequestInput:GetTall()-45 )
			Button2:SetText( "" )
