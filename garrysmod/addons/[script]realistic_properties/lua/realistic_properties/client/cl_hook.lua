            ...
            if LocalPlayer():GetPos():DistToSqr(pos) < 200*200 then 
                cam.Start3D2D(pos + ang:Up(), ang, 0.03)
                    if isstring(ent:getKeysDoorGroup()) then 
                        draw.SimpleTextOutlined(Realistic_Properties.Lang[73][Realistic_Properties.Language], "rps_font_10", 0, 140, RealisticPropertiesColorAlpha, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, RealisticPropertiesColorAlpha)
                    ...
                    if string.len(ent:GetNWString("PropertiesName")) > 0  then 
                        if isstring(ent:GetNWString("PropertiesName")) && ent:GetNWString("PropertiesName") != "disable" then 
                            if ent:GetNWString("PropertiesName") != "" then 
                                draw.SimpleTextOutlined(ent:GetNWString("PropertiesName"), "rps_font_10", 20, 140, RealisticPropertiesColorAlpha, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, RealisticPropertiesColorAlpha)
                                if ent:GetNWBool("PropertiesBuy") == true then 
                                    draw.SimpleTextOutlined(Realistic_Properties.Lang[41][Realistic_Properties.Language], "rps_font_10", 0, 20, RealisticPropertiesColorAlpha, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, RealisticPropertiesColorAlpha)
                                else
                                    draw.SimpleTextOutlined(Realistic_Properties.Lang[42][Realistic_Properties.Language], "rps_font_10", 0, 20, RealisticPropertiesColorAlpha, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, RealisticPropertiesColorAlpha)
                                end 
                            end 
                        end 
                        if not Realistic_Properties.PlayerSeeOwner then 
                            if isstring(ent:GetNWString("PropertiesOwner")) then 
                                if ent:GetNWString("PropertiesOwner") != "" then 
                                    if Realistic_Properties.PoliceJob[team.GetName(LocalPlayer():Team())] then 
                                        draw.SimpleTextOutlined("( "..Realistic_Properties.Lang[72][Realistic_Properties.Language].." : "..ent:GetNWString("PropertiesOwner"), "rps_font_105", 0, 260, RealisticPropertiesColorAlpha, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, RealisticPropertiesColorAlpha)
                                    end 
                                end 
                            end 
                        else 
