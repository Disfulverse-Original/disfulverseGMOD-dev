...
if CLIENT then
	...
	language.Add("tool.realistic_properties_tool.name", "Properties Setup")
	language.Add("tool.realistic_properties_tool.desc", "Create or modify Properties in your server")
	language.Add("tool.realistic_properties_tool.left", "Left-Click to configure Delivery Box, Cameras & Zone" )
	language.Add("tool.realistic_properties_tool.right", "Right-Click while aiming NPC or Computer to open the admin menu" )
	language.Add("tool.realistic_properties_tool.use", "USE to return to the previous step" )
	language.Add("tool.realistic_properties_tool.reload", "RELOAD to go to the next step" )
end
...
function TOOL:DrawToolScreen( w, h )
	if CLIENT then 
		...
		if IsValid(self.RWSEnt) then 
			draw.SimpleText( Realistic_Properties.Lang[2][Realistic_Properties.Language].." (1)", "rps_font_12", w / 2, h / 5.7, Realistic_Properties.Colors["green"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		else
			draw.SimpleText( Realistic_Properties.Lang[2][Realistic_Properties.Language], "rps_font_12", w / 2, h / 5.7, Realistic_Properties.Colors["graywhite"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end 
		if IsEntity(self.RealisticPropertiesCamPos[3]) then 
			draw.SimpleText( Realistic_Properties.Lang[3][Realistic_Properties.Language].." ("..#self.RealisticPropertiesCamPos.."/3)", "rps_font_12", w / 2, h / 2.4, Realistic_Properties.Colors["green"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		elseif IsEntity(self.RealisticPropertiesCamPos[1]) then 
			draw.SimpleText( Realistic_Properties.Lang[3][Realistic_Properties.Language].." ("..#self.RealisticPropertiesCamPos.."/3)", "rps_font_12", w / 2, h / 2.4, Realistic_Properties.Colors["graywhite"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		else
			draw.SimpleText( Realistic_Properties.Lang[3][Realistic_Properties.Language].." (0/3)", "rps_font_12", w / 2, h / 2.4, Realistic_Properties.Colors["graywhite"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end 
		if isvector(RealisticPropertiesPos1) && isvector(RealisticPropertiesPosGhost2) then 
			draw.SimpleText( Realistic_Properties.Lang[4][Realistic_Properties.Language], "rps_font_12", w / 2, h / 1.57, Realistic_Properties.Colors["green"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		else 
			draw.SimpleText( Realistic_Properties.Lang[4][Realistic_Properties.Language], "rps_font_12", w / 2, h / 1.57, Realistic_Properties.Colors["graywhite"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end 
		if IsEntity(self.RealisticPropertiesCamPos[3]) && IsValid(self.RWSEnt) && istable(RealisticPropertiesTableDoors) && #RealisticPropertiesTableDoorsData == 0 then  
			if #RealisticPropertiesTableDoors != 0 then 
				draw.SimpleText( Realistic_Properties.Lang[68][Realistic_Properties.Language], "rps_font_12", w / 2, h / 1.16, Realistic_Properties.Colors["green"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			else 
				draw.SimpleText( Realistic_Properties.Lang[68][Realistic_Properties.Language], "rps_font_12", w / 2, h / 1.16, Realistic_Properties.Colors["graywhite"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			end 
		else 
			draw.SimpleText( Realistic_Properties.Lang[68][Realistic_Properties.Language], "rps_font_12", w / 2, h / 1.16, Realistic_Properties.Colors["graywhite"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end 
				...
				if IsEntity(self.RealisticPropertiesCamPos[3]) then  
					if isvector(RealisticPropertiesPos1) && isvector(RealisticPropertiesPosGhost2) then 
						if #RealisticPropertiesTableDoorsData == 0 then 
							...
							RealisticPropertiesNotify( Realistic_Properties.Lang[51][Realistic_Properties.Language], 3 )
						end 
					end 
				end 
			end 
...
function TOOL.BuildCPanel( CPanel )
	...
	Text = "Save all Entities in the Data" })
	CPanel:Button("Save Entities", "rps_saveentities")

	CPanel:AddControl("label", {
	Text = "Remove all Entities in The Data" })  
	CPanel:Button("Remove Entities Data", "rps_removedata")

	CPanel:AddControl("label", {
	Text = "Remove all Entities in The Map" })
	CPanel:Button("Remove Entities Map", "rps_cleaupentities")

	CPanel:AddControl("label", {
	Text = "Reload all Entities in The Map" })
	CPanel:Button("Reload Entities Map", "rps_reloadentities")
end