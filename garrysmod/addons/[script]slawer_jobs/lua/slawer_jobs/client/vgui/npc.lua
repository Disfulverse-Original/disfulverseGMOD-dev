...
function PANEL:Init()
    ...
    dInfo.dLevel = Label("", dInfo)
    dInfo.dLevel:SetFont("Slawer.Jobs:SB35")
    dInfo.dLevel:SetTextColor(THEME.Texts)
    dInfo.dLevel:SetContentAlignment(6)
    dInfo.dLevel:SetSize(dInfo:GetWide() * 0.25, dInfo:GetTall() * 0.045)
    dInfo.dLevel:SetPos(dInfo:GetWide() * 0, dInfo:GetTall() * 0.085)
...
function PANEL:ShowJob(iJob)
    ...
    elseif BRICKS_SERVER.Func.IsSubModuleEnabled( "essentials", "levelling" ) and tJob.level then
        if ( (LocalPlayer().BRS_LEVEL or 0) < BRICKS_SERVER.CONFIG.LEVELING.JobLevels[RPExtraTeams[tJob].command or "error"] ) then
            sRestrict = sRestrict .. Slawer.Jobs:L("needLevel"):format(tJob.level) .. "\n"
        end
    end
    ...
    // Level
    local jobCommand = tJob.command or "error"
    local jobLevel = BRICKS_SERVER.CONFIG.LEVELING.JobLevels[RPExtraTeams[iJob].command or "Unknown"]

    -- If jobLevel is nil, set it to 0
    if jobLevel == nil then
        jobLevel = 0
    end

    --print(jobLevel)
    function dInfo.dLevel:Paint(w, h)
        -- Draw background or any other visual elements first
        surface.SetDrawColor(AAS.Colors["disfulpurple"])
        surface.DrawTexturedRect(0, 0, w, h)  -- Assuming you want to cover the entire label with a texture

        -- Draw text
        surface.SetFont("Slawer.Jobs:SB35")
        surface.SetTextColor(color_white)
        surface.SetTextPos(0, 5)  -- Adjust the position based on your requirements
        surface.DrawText("| Уровень " .. tostring(jobLevel))  -- Concatenate jobLevel with the text
    end
