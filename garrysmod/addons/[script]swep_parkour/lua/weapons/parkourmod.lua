...
if CLIENT then
	...
	pm_disablemotionpp = CreateClientConVar( 'pm_disablemotionpp', '1', true )
	
	pm_hudpos = CreateClientConVar( 'pm_hudpos', '0', true )
end

pm_sv_disableadr = CreateConVar( 'pm_sv_disableadr', '1', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )
pm_sv_disablehud = CreateConVar( 'pm_sv_disablehud', '1', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )
pm_sv_disableturn = CreateConVar( 'pm_sv_disableturn', '0', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )
pm_sv_maxspeed = CreateConVar( 'pm_sv_maxspeed', '340', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )
pm_sv_maxjump = CreateConVar( 'pm_sv_maxjump', '450', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )
pm_sv_maxastabdist = CreateConVar( 'pm_sv_maxastabdist', '500', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )
pm_sv_climbvel = CreateConVar( 'pm_sv_climbvel', '150', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )
pm_sv_disabledoorbust = CreateConVar( 'pm_sv_disabledoorbust', '1', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )
pm_sv_disablestabing = CreateConVar( 'pm_sv_disablestabing', '1', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )
pm_sv_disablehealthregen = CreateConVar( 'pm_sv_disablehealthregen', '1', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )
pm_viewbob = CreateConVar( 'pm_viewbob', '1', { FCVAR_REPLICATED, FCVAR_ARCHIVE } )

SWEP.PrintName			= 'Руки Бегущего'
SWEP.Category			= ""
...
function SWEP:PrimaryAttack()
	...
	ply:EmitSound( 'parkourmod/punch_miss' .. math.random( 1, 4 ) .. '.wav' )
	...
	if tr.Hit and SERVER then
		ply:EmitSound( 'parkourmod/punch_out' .. math.random( 1, 6 ) .. '.wav' )
		...
		if IsValid( tr.Entity ) and ( tr.Entity:IsNPC() || tr.Entity:IsPlayer() || tr.Entity:Health() > 0 ) then
			timer.Simple( 0.1, function() if !IsValid( self ) or !IsValid( ply ) or !IsValid( tr.Entity ) then return end tr.Entity:TakeDamage( math.random( 3, 5 ), ply, self ) end )
		end
...
function SWEP:InitWallRun()
	local ply = self.Owner
	if !IsValid( ply ) or ply:GetVelocity():Length() < 300 then return end
...
function SWEP:AttemptOBSJump()
	...
	if tr.Hit and ( tr.Entity:IsWorld() or tr.Entity:GetClass() == 'prop_physics' or ( tr.Entity:IsPlayer() and tr.Entity:KeyDown( IN_DUCK ) ) ) and ply:IsOnGround() and !tr_fwrd.Hit and ply.NextOBSJump < CurTime() then
		...
		ply:EmitSound( 'parkourmod/impact_obs1.wav' )
		timer.Simple( 0.3, function() if !IsValid( ply ) then return end ply:EmitSound( 'parkourmod/impact_obs2.wav' ) ply:ViewPunch( Angle( -15, -12, 5 ) ) end )
	...
	if tr.Hit and !tr_up_fwrd.Hit and ( tr.Entity:IsWorld() or tr.Entity:GetClass() == 'prop_physics' ) and ply:IsOnGround() and !tr_up.Hit and ply.NextOBSJump < CurTime() then
		...
		ply:EmitSound( 'parkourmod/impact_obs2.wav' )
		timer.Simple( 0.2, function() ply:ViewPunch( Angle( -6, 2, -6 ) ) ply:EmitSound( 'parkourmod/impact_obs6.wav' ) end )
...
function SWEP:Init180Turn()
	...
	ply:EmitSound( 'parkourmod/180_Turn' .. math.random( 1, 2 ) .. '.wav' )
end
		...
		if ply:KeyDown( IN_USE ) then
			...
			hook.Add( 'Think', 'ParkourThinkOnStabTarget_' .. ply:EntIndex(), function()
				...
				if ply:GetPos():Distance( target:GetPos() + Vector( 0, 0, 45 ) ) < 80 then
					...
					ply.AirStabAnimTimer = CurTime() + 0.6
