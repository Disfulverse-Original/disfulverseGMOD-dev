...
hook.Add("yaws.warns.warned", "yaws.discord.webhook", function(player, admin, reason, points)
                ...
                :next(function(admin)
                    ...
                    if(YAWS.ManualConfig.Discord.Method == "CHTTP") then 
                        CHTTP({
                            ...
                            success = function(code, body, headers)
                                ...
                                YAWS.Core.Print("[Discord] POST request was successful.")
                                YAWS.Core.Print("[Discord] Response Code: " .. code)
                                YAWS.Core.Print("[Discord] Response Headers: " .. util.TableToJSON(headers))
                                YAWS.Core.Print("[Discord] Response Body: " .. body)
                            end,
                            failed = function(reason)
                                YAWS.Core.Print("[Discord] POST request failed!")
                                YAWS.Core.Print("[Discord] Reason: " .. reason)
                            end 
                        })
                    end 
                    ...
                    if(YAWS.ManualConfig.Discord.Method == "RELAY") then 
                        http.Post(YAWS.ManualConfig.Discord.RelayURL, {
                            ...
                            YAWS.Core.Print("[Discord] POST request failed!")
                            YAWS.Core.Print("[Discord] Reason: " .. error)
                        end, {
