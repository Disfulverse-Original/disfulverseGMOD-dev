...
function PANEL:Refresh()
    ...
    local rewards = BOTCHED.FUNC.GetChangedVariable( "REWARDS", "TimeRewards" ) or BOTCHED.CONFIGMETA.REWARDS:GetConfigValue( "TimeRewards" )
    ...
    for k, v in pairs( sortedRewards ) do
        local timeRewardKey, timeRewardCfg = v[2], v[3]

        self:CreateTimeRewardSlot( self.grid, timeRewardCfg, slotSize, slotSize*1.2, function()
            self:CreateRewardPopup( timeRewardKey, rewards )
        end )
    end
    ...
    addNewButton.DoClick = function()
        BOTCHED.FUNC.DermaStringRequest( "Enter a unique reward ID, no spaces, no capitals.", "REWARD CREATION", "uniqueid", false, function( value )
            if( rewards[value] ) then 
                BOTCHED.FUNC.DermaMessage( "A reward already exists with this ID.", "CREATION ERROR" )
                return
            end

            rewards[value] = {
                Time = sortedRewards[#sortedRewards][1]+1,
                RewardType = "Money",
                RewardValue = 1000
            }
    
            BOTCHED.FUNC.RequestConfigChange( "REWARDS", "TimeRewards", rewards )
            self:Refresh()
        end )
    end
...
function PANEL:CreateTimeRewardSlot( parent, timeRewardCfg, w, h, doClick )
    ...
    rewardPanel.UpdateRewardInfo = function( self2, timeRewardCfg )
        ...
        if( timeRewardCfg.RewardType == "Items" ) then
            if( BOTCHED.CONFIG.LOCKER.Items[timeRewardCfg.RewardValue[1]] ) then
                itemInfo = BOTCHED.CONFIG.LOCKER.Items[timeRewardCfg.RewardValue[1]]
            end

            amount = timeRewardCfg.RewardValue[2]
        elseif( BOTCHED.DEVCONFIG.RewardTypes[timeRewardCfg.RewardType] ) then
            local rewardCfg = BOTCHED.DEVCONFIG.RewardTypes[timeRewardCfg.RewardType]
            itemInfo = {
                Name = rewardCfg.Name, 
            ...
            amount = timeRewardCfg.RewardValue
        end
        ...
        rewardCover.Paint = function( self2, w, h ) 
            draw.SimpleText( BOTCHED.FUNC.FormatLetterTime( timeRewardCfg.Time ), "MontserratBold17", w/2, BOTCHED.FUNC.ScreenScale( 25 ), BOTCHED.FUNC.GetTheme( 4, 75 ), TEXT_ALIGN_CENTER, 0 )
        end
    end
...
function PANEL:CreateRewardPopup( rewardKey, rewards )
    ...
    popup:SetHeader( "TIME REWARD CONFIG" )
    ...
    itemPanel = self:CreateTimeRewardSlot( popup, configItem, itemPanelWide, itemPanelWide*1.2 )
    ...
    -- TIME --
    local timeField = popup:AddField( "Time", "The play time required.", Material( "botched/icons/time_played.png" ) )
    timeField:SetExtraHeight( BOTCHED.FUNC.ScreenScale( 60 ) )

    local timeEntry = vgui.Create( "botched_numberwang", timeField )
    timeEntry:Dock( TOP )
    timeEntry:DockMargin( margin10, margin10, margin10, margin10 )
    timeEntry:SetTall( BOTCHED.FUNC.ScreenScale( 40 ) )
    timeEntry:SetBackColor( BOTCHED.FUNC.GetTheme( 1 ) )
    timeEntry:SetHighlightColor( BOTCHED.FUNC.GetTheme( 2, 25 ) )
    timeEntry:SetValue( configItem.Time )
    timeEntry.OnChange = function( self2 )
        ChangeItemVariable( "Time", self2:GetValue() )
...
vgui.Register( "botched_config_timerewards", PANEL, "DPanel" )