...
SWEP.PrintName = "Проверка нелегала"
SWEP.Spawnable = true
SWEP.AdminOnly = true
...
local function hasWeaponLicense(player)
    return player:getDarkRPVar("HasGunlicense")
end
...
function SWEP:GetStrippableWeapons(ent, callback)
    CAMI.PlayerHasAccess(ent, "DarkRP_GetAdminWeapons", function(access)
        for _, v in ipairs(ent:GetWeapons()) do
            ...
            local hasLicense = hasWeaponLicense(ent)

            -- Exclude the weapon if the player has a license
