...
function plyMeta:warrant(warranter, reason)
    --if warranter:isCP() then DarkRP.notify(self, 1, 5, "Вы не можете запросить ордер на сотрудника полиции.") return end
...
function plyMeta:wanted(actor, reason, time)
    --if actor:isCP() then DarkRP.notify(self, 1, 5, "Вы не можете подать в розыск сотрудника полиции.") return end
...
function plyMeta:unWanted(actor)
    if self:getDarkRPVar("wanted") == nil then return end
